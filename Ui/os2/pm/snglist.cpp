...
void SongList::ReLocate(iterator &T)
{
   ...
   if (TrueT->Parent == 0 || TrueT->Parent->UnSorted == 1)
      return;
...
SongList::iterator SongList::Link(Node *Parent,Node *Target,iterator Insert)
{
   ...
   // Not sorted add at the end.
   if (Parent->UnSorted == 1 && Insert == 0)
   {
      if (Parent->Child != 0)
      {
         for(Insert = Parent->Child; Insert != 0; Insert++);
         Insert--;
      }
   }
...
{
   // Never include head nodes in paths
   if (Base->Parent != 0 && Base->Parent->Parent != 0)
