...
say " Module Player Song Changer V1.0                                          Ethos"
   call OpenPlayer
   if Result <> 0 then do
      say "Error" Error
      return
   ...
   if (arg(1) = '') then do
      say "Require a substring to match in the songlist."
      return
   end

   if CallPlayer("Play","Song",arg(1)) <> 0 then do
      say "Error" Error
      return
   end
   Say "Jumped to" Result.2
